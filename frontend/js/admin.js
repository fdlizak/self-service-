...
class AdminDashboard {
  ...
  renderQueueData(queue) {
    ...
    document.getElementById("waiting-queue").innerHTML = waitingQueue
      .map(
        (q) => `
    ...
    document.getElementById("completed-queue").innerHTML = completedQueue
      .map(
        (q) => `
            <div class="queue-item">
                <div class="queue-number">${q.queue_number}</div>
  ...
  async addProduct(event) {
    ...
    try {
      if (this.editingProductId) {
        await this.apiCall(`/admin/products/${this.editingProductId}`, "PUT", product);
      ...
      console.error(error);
      this.showAlert("Gagal menyimpan produk", "error");
    }
  }

  async editProduct(productId) {
    try {
      const res = await this.apiCall(`/products/${productId}`);
      ...
      console.error(error);
      this.showAlert("Gagal memuat data produk", "error");
    }
  }
